...
class GalleryImagesViewModel(context: Context) : ViewModel() {
    ...
    private suspend fun getImagesFromDevice(context: Context): List<ImageData> {
        ...
        cursor?.use {
            ...
            while (it.moveToNext()) {
                ...
                val uri = getMediaFileUri(context, File(data))
//                val isFileExists = fileExists(context, File(data))
                val isFileExists = File(data)
                Log.d("TAG", "getImagesFromDevice: ${data.toString()}")
